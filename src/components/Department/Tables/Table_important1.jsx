...
function Table_important({ api, langState, data, t, i18n, dataSeo }) {
    ...
    const statusImgTemplate = (data) => {
        ...
        return (<AvatarGroup style={{ direction: 'ltr' }}>
            {data.photos.map((Element, index) => (
                ...
                index < 5 &&
                <Avatar image={imgSrc(Element.photo)} size="large" shape="circle" key={index} />
            ))}
            {/* هشوف فاضل كام صوره بعد ال 5 صور الي ظهرتهم  */}
            {data.photos.length - 5 >= 1 &&
                <Avatar label={`+${data.photos.length - 5}`} shape="circle" size="large" />}
        </AvatarGroup>);
    };
        ...
        <section className={`${style.Tables} py-3 py-sm-5`}   >
            <div className='container'>
                ...
                <DataTable value={data} size="small"
                    ...
                    globalFilterFields={['notes_ar', 'notes_en', 'date', 'number']}
                    ...
                    <Column field={`${langState ? 'notes_ar' : 'notes_en'}`} header=" الوصف " sortable className={`${langState && 'text-end'}`} style={{ whiteSpace: 'normal' }}  ></Column>
...
export default Table_important
