...
function BoardStructure({ api, t, i18n, units, langState, textDescription }) {
    const [dataSeo, setdataSeo] = useState([])
    const { name } = useParams();
    const doctorKind = [0, 1, 2];
    ...
    // useEffect fun
    useEffect(() => {
        setdataSeo({ title: `مجلس ادارة  ${name} `, description: textDescription, keywords: '....' })
        window.scrollTo(0, 0);
