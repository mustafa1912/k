...
import style_ar from './404.module.css'
import style_en from './404_en.module.css'
import { NavLink } from 'react-router-dom'

function NotFound({ t, i18n, data }) {
  let style = i18n.language === 'ar' ? style_ar : style_en
  return (
    <React.Fragment>
      <div className={`${style.hamburger_menu}`}  >
        <button className={`${style.burger}`} data-state="closed">
          <span className={`${style.span}`}></span>
          <span className={`${style.span}`}></span>
          <span className={`${style.span}`}></span>
        </button>
      </div>
      <nav data-state={`${style.closed}`} >
        ...
        <div className="container">
          <div className="row">
            ...
            <div className="col-md-6 align-self-center text-center">
              <h1 className={`${style.head} text-center`}>404</h1>
              <h2 className={`text-center`}> الصفحة غير موجوده </h2>
              <p className={`text-center mb-4`}>الصفحة التي تبحث عنها غير موجودة. ولكن يمكنك الضغط فوق الزر أدناه للعودة إلى الصفحة الرئيسية.
              </p>
              <NavLink className={`${style.btn} mt-3`} to='/'> الصفحة الرئيسية</NavLink>
            </div>
          </div>
        </div>
      </main>
